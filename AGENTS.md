...
- OpenCode provider/model data is fetched via `src/opencode/opencode-models-service.ts` using `execSync` to run `opencode models`. It uses `isBinaryAvailableOnPath` (same as agent detection) to check if opencode is installed, and caches results for 5 minutes. The `opencode models` output format is `provider/model` lines, which are parsed to extract unique providers and per-provider model lists. Backend TRPC endpoints (`opencode.listProviders`, `opencode.listModels`) delegate to this service, and the frontend hooks (`useOpenCodeProviders`, `useOpenCodeModels`) call them via the `runtime-config-query.ts` helpers. The web-ui types (`RuntimeOpenCodeProvider`, `RuntimeOpenCodeModel`) come from `@runtime-contract` (which resolves to `src/core/api-contract.ts`) via `web-ui/src/runtime/types.ts` re-exports.
- **AgentSelector mount overwrites customArgs on edit**: `AgentSelector` (`web-ui/src/components/AgentSelector.tsx`) always initializes its internal state with hardcoded defaults (e.g., `OPENCODE_AGENTS[0]!.id`) and has a `useEffect` that fires `onSelectionChange` on mount, which overwrites the parent's `editTaskCustomArgs` with defaults. When opening a task for editing, this silently drops any previously configured agent/model overrides. Fix: parse existing `customArgs` for initial state and skip `onSelectionChange` on mount via a `useRef`.
